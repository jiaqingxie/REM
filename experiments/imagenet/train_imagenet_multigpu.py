...
def forward_all(model,
    ...
    Optionally performs a one-sided trimmed mean on the negative energies
...
def train_loop(rank, world_size, argv):
    ...
    # -----------------------------------------------------------------------
    data_root = os.environ.get("IMAGENET32_PATH")
    if not data_root:
    ...
    # Match the CIFAR setup: when CD loss is active all parameters participate,
    # otherwise some CD-specific heads go unused and DDP must tolerate that.
        ...
        for step in range(start_step, FLAGS.total_steps + 1):
            ...
            if rank == 0 and FLAGS.save_step > 0 and step % FLAGS.save_step == 0 and step > 0:
                ...
                ckpt_path = os.path.join(
                    savedir, f"{FLAGS.model}_imagenet32_weights_step_{step}.pt"
                )
                torch.save(
                    {
